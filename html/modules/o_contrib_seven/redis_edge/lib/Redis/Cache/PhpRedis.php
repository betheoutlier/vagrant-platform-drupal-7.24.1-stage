 ...
 * Some random notes:
 *
 *  * This makes no sense, but sometime PhpRedis will return a strict false
 *    when running the MULTI comand, which is NOT POSSIBLE reading the Redis
 ...
 *
 *    But seriously, it happens. This might cause serious troubles because
 *    you might end up not UNWATCHing some keys and cause the whole process
 *    after that to fail.
 *
 *    Hence the very offuscated transaction handling in this class which is
 *    way more simple (and more close to Redis server spec) in Predis lock
 *    backend implementation.
 *
 *  * PhpRedis offends its own method signatures documented in the official
 *    documentation and may arbitrarily return a Redis object instance
 *    instead of key values or a strict false when running the SET command.
 *    Serious WTF here but once again this code attempts to detect that and
 *    work arround.
 */
